...
EOH
    ...
    installed_release=$(kubectl get cm -n tekton-pipelines pipelines-info -o jsonpath='{.data.version}')
    if [[ ${installed_release} != "v0.45.0" ]]; then
...
if [[ -f ${repo_root}/pipeline/custom_pre_workspace.sh ]]; then
